...
//com.itwill.spring2.service 패키비와 그 하위 패키지를 스캔(검색)하고 있음.
@Service //-> 스프링 컨테이너에서 서비스 컴포넌트 객체를 생성하고 관리(필용한 곳에 주입).
@RequiredArgsConstructor //2.(2) final로 선언된 필드를 초기화하는 생성자를 자동으로 만들어줌.
@Slf4j
public class PostService {
    
    //의존성 주입(DI: Dependency Injection): 1번, 2번 중 하나 사용
...
//    @Autowired private PostRepository postRepository; //1. 필드에 의한 의존성 주입
    ...
    private final PostRepository postRepository; //final 반드시 변수는 초기화해야됨/ 2.(1) 생성자에 의한 의존성주입
