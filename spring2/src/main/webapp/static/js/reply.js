...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const deleteReply = (e) => {
        ...
        //삭제 요청 URL
        const reUrl = `/spring2/api/reply/${id}`;
        //삭제 요청을 Ajax 방식으로 보냄
        axios.delete(reUrl)
            .then((response) => {
    ...
    const showupdateModal = (e) => {
        //console.lig(e) // e: 이벤트 객체
        //console.log(e.target); //e.target: 이벤트가 발생한 타켓. 여기서는 수정 버튼
        const id = e.target.getAttribute('data-id');
    ...
    const makeReplyElements = (data) => {
        ...
        replies.innerHTML = ''; //<div>의 컨텐트를 지움. 새로 만들겠다.
        
        let htmlStr = ''; //const와 let의 차이점: 변경여부. 문자열 계속 붙일거라 let
        ...
        //deleteButtons->element들의 배열이됨 -> for문 사용
    ...
    const getRepliesWithPostId = async () => {
        ...
        try {
            ...
            console.log(response);
            //댓글 개수 업데이트 & 댓글 목록 보여주기(댓글 배열 필요->response가 배열 가지고 있음. data가 배열임)
